...
// ============================================================================
// 概要:
// - サーバー起動時に実行したい初期化処理（監視/環境検証）を集約する。
//
// 責務:
// - Node.js runtime（server）でのみ、Sentryサーバ初期化を実行する。
// - Next.js の request error hook を Sentry に接続する。
...
export async function register() {
  ...
  if (process.env.NEXT_RUNTIME === "nodejs") {
    ...
    // 2) server-only モジュールを nodejs の時だけ読み込む
...
// Next.js が提供する request error hook を Sentry に接続する（公式推奨）
