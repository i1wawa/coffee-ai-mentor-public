<!-- docs/adrs/0004-adopt-hybrid-secret-management-for-ci-cd-and-cloud-run.md -->
...
- ビルド時:
  ...
  - ビルド時のみ必要なCI専用シークレット（例: Sentry source maps アップロード用の `SENTRY_AUTH_TOKEN`）
- 実行時:
...
- Docker build（ビルド時）で、シークレットを ARG/ENV で渡すと、イメージ層、ビルドログ、キャッシュ、履歴に残るリスクが高い
...
- Option C
  - Pros
    ...
    - Docker buildでSecretsをARG/ENVに残さない設計を徹底できる
  - Cons
...
- GitHub Actions（GitHub Environments）に置くもの
  ...
  - Secrets（CI専用Secrets）
    - 例: `SENTRY_AUTH_TOKEN`（Sentry source mapsアップロード用）
...
2. Docker build時のシークレット注入ルール

- 原則:
  - Docker buildではシークレットをARG/ENVで渡さない
- 例外（ビルド時のみ必要なCI専用Secrets）:
  - BuildKit secret mount（`RUN --mount=type=secret`）で一時注入して利用する
  - イメージ層・最終イメージ・Runtime環境変数に残さない
  - 用途例: `withSentryConfig` による source maps アップロード用 `SENTRY_AUTH_TOKEN`

## Positive Consequences

- 実行時SecretsはSecret Managerに集中し、監査・ローテーション・棚卸しがしやすくなる
- CIは鍵レス（WIF/OIDC）で運用でき、長期鍵漏えいリスクを低減できる
- Docker buildでSecretsをARG/ENVに入れないルールが明確になり、イメージ層やキャッシュへの混入事故を防げる
